...
pub fn new_image_plugin(ctx: &mut zmq::Context) {
    ...
    let sync = ctx.socket(zmq::REQ).expect("New Image plugin could not create sync socket.");
    sync.connect("inproc://sync-5000").expect("New Image plugin could not connect to sync socket.");
    println!("New Image plugin connected to sync socket.");

    thread::spawn(move || {
        // connect to and send sync message on sync socket
        let msg = "ready";
        sync.send(msg, 0).expect("New Image plugin could not send sync message");
        println!("New Image plugin sent sync message.");
        // wait for reply from engine
        let _msg = sync.recv_msg(0).expect("New Image plugin got error trying to receive sync reply");
        ...
        while count < 5 {
            ...
            send_new_image_event(&mut new_messages,
                &mut bldr,
                &uuid,
                &"png".to_string(),
                &Vec::<u8>::new()
