...
pub fn image_scored_plugin(ctx: &mut zmq::Context) {
    ...
    let sync = ctx.socket(zmq::REQ).expect("Image scored plugin could not create sync socket.");
    sync.connect("inproc://sync-5001").expect("Image scored plugin could not connect to sync socket.");
    println!("Image scored plugin connected to sync socket.");

    thread::spawn(move || {
        // connect to and send sync message on sync socket
        let msg = "ready";
        sync.send(msg, 0).expect("Image scored plugin could not send sync message");
        println!("Image scored plugin sent sync message.");
        // wait for reply from engine
        let _msg = sync.recv_msg(0).expect("Image scored plugin got error trying to receive sync reply");
        ...
        while count < 5 {
            ...
            let image_uuid = event.event_as_new_image_event().unwrap().image_uuid().unwrap();
            println!("Image scored plugin got New Image event for image {}", image_uuid);
            ...
            probability: prob });
            send_image_scored_event(&mut new_messages, &mut bldr,  &image_uuid, scores).expect("Could not send image scored event");
            count += 1;
            println!("Image scored plugin sent Image Scored event for image: {}; prob: {}", image_uuid, prob);
        }
    });
}
