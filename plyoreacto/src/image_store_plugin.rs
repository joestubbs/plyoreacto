...
pub fn image_stored_plugin(ctx: &mut zmq::Context) {
    ...
    let sync = ctx.socket(zmq::REQ).expect("Image stored plugin could not create sync socket.");
    sync.connect("inproc://sync-5002").expect("Image stored plugin could not connect to sync socket.");
    println!("Image stored plugin connected to sync socket.");

    thread::spawn(move || {
        // connect to and send sync message on sync socket
        let msg = "ready";
        sync.send(msg, 0).expect("Image stored plugin could not send sync message");
        println!("Image stored plugin sent sync message.");
        // wait for reply from engine
        let _msg = sync.recv_msg(0).expect("Image stored plugin got error trying to receive sync reply");
        ...
        while count < 5 {
            ...
            let event_type = event.event_type().variant_name().expect("could not get event type");
            if event_type != "ImageScoredEvent" {
                continue;
            }            
            let image_uuid = event.event_as_image_scored_event().unwrap().image_uuid().unwrap();
            println!("Image stored plugin got ImageScored event for image {}", image_uuid);
            ...
            // delete the image
            if prob < 0.5 {
                send_image_deleted_event(&mut new_messages, &mut bldr, image_uuid).expect("could not sent image deleted event");
                println!("Image stored plugin sent an image deleted event for image {}", image_uuid);
            }else {
                send_image_stored_event(&mut new_messages, &mut bldr, image_uuid).expect("could not sent image deleted event");
                println!("Image stored plugin sent an image stored event for image {}", image_uuid);
            }
            count += 1;
        }
    });
