...
use zmq::Socket;
use crate::events_generated::events::{EventArgs, EventType, Event, ImageLabelScore, ImageScoredEventArgs, ImageScoredEvent};

use super::events_generated::events::{NewImageEvent, NewImageEventArgs, ImageDeletedEvent, 
    ImageDeletedEventArgs, ImageStoredEvent, 
    ImageStoredEventArgs, ImageLabelScoreArgs, root_as_event};


pub fn send_new_image_event(msg_socket: &mut Socket, bldr: &mut FlatBufferBuilder, image_uuid: &str, image_format: &str, image: &Vec<u8>) ->  Result<(), std::io::Error>{
    ...
    msg_socket.send(bldr.finished_data(), 0).expect("could not send new image event over zmq socket");
...
pub fn send_image_scored_event(msg_socket: &mut Socket, bldr: &mut FlatBufferBuilder, image_uuid: &str, scores: Vec<ImageScore>) ->  Result<(), std::io::Error>{
    ...
    for score in scores {
        ...
        let im_score = ImageLabelScore::create(bldr, 
            &ImageLabelScoreArgs {
                label: label,
    ...
    // send the new_event message over the messages socket
    msg_socket.send(bldr.finished_data(), 0).expect("could not send image scored event over zmq socket");
...
pub fn send_image_stored_event(msg_socket: &mut Socket, bldr: &mut FlatBufferBuilder, image_uuid: &str) ->  Result<(), std::io::Error>{
    ...
    // send the new_event message over the messages socket
    msg_socket.send(bldr.finished_data(), 0).expect("could not send image stored event over zmq socket");

    Ok(())

}

pub fn send_image_deleted_event(msg_socket: &mut Socket, bldr: &mut FlatBufferBuilder, image_uuid: &str) ->  Result<(), std::io::Error>{
    ...
    bldr.finish(event, None); 
    // send the new_event message over the messages socket
    msg_socket.send(bldr.finished_data(), 0).expect("could not send image deleted event over zmq socket");
...
mod test {
    ...
    fn test_write_new_image_event_to_file() -> std::io::Result<()> {
        ...
        let mut file = OpenOptions::new().write(true).open("/home/jstubbs/Documents/new_image1.msg")?;
        file.write_all(bldr.finished_data()).expect("could not write data");
    ...
    fn test_read_new_image_event_from_file() -> std::io::Result<()> {
        ...
        let variant = event_type.variant_name().expect("could not get event variant");
        dbg!(variant);
        assert_eq!("NewImageEvent", variant);

        let new_image_event = msg.event_as_new_image_event().expect("could not cast to NewImageEvent");
        let image_uuid = new_image_event.image_uuid().expect("could not get image_uuid from event");
        dbg!(image_uuid);
    ...
    fn test_write_image_stored_event_to_file() -> std::io::Result<()> {
        ...
        // write data to a file
        let mut file = OpenOptions::new().write(true).open("/home/jstubbs/Documents/image_stored2.msg")?;
        file.write_all(bldr.finished_data()).expect("could not write data");
    ...
    fn test_read_image_stored_event_from_file() -> std::io::Result<()> {
        ...
        let stored_event = msg.event_as_image_stored_event().expect("could not cast to ImageStoredEvent");
        let image_uuid = stored_event.image_uuid().expect("could not get image_uuid from event");
        dbg!(image_uuid);
    ...
    fn test_write_image_scored_event_to_file() -> std::io::Result<()> {
        ...
        scores.push(ImageScore { label: "laborador".to_string(),
            probability: 0.98 });
        scores.push(ImageScore { label: "golden retriever".to_string(), 
            probability: 0.02 });

        let mut image_label_scores = Vec::<WIPOffset<ImageLabelScore>>::new();
        for score in scores {
            let label = Some(bldr.create_string(&score.label));
            let im_score = ImageLabelScore::create(&mut bldr, 
                &ImageLabelScoreArgs {
                    label: label,
                    probability: score.probability,
            });
        ...
        // write data to a file
        let mut file = OpenOptions::new().write(true).open("/home/jstubbs/Documents/image_scored1.msg")?;
        file.write_all(bldr.finished_data()).expect("could not write data");
    ...
    fn test_read_image_scored_event_from_file() -> std::io::Result<()> {
        ...
        let scored_event = msg.event_as_image_scored_event().expect("could not cast to ImageScoredEvent");
        let image_uuid = scored_event.image_uuid().expect("could not get image_uuid from event");
        dbg!(image_uuid);
    ...
    fn test_write_image_deleted_event_to_file() -> std::io::Result<()> {
        ...
        // write data to a file
        let mut file = OpenOptions::new().write(true).open("/home/jstubbs/Documents/image_deleted2.msg")?;
        file.write_all(bldr.finished_data()).expect("could not write data");
    ...
    fn test_read_image_deleted_event_from_file() -> std::io::Result<()> {
        ...
        let deleted_event = msg.event_as_image_deleted_event().expect("could not cast to ImageDeletedEvent");
        let image_uuid = deleted_event.image_uuid().expect("could not get image_uuid from event");
        dbg!(image_uuid);
